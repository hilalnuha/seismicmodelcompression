...
close
load('datatest.mat', 'data594Br')

%L=1501;
M=10;% number of model (We used M=20)
[baris kolom]=size(data594Br);


NN=10;% number of seismic traces in the dataset (we used NN=kolom);
models=zeros(baris,M,NN);
varenergy=zeros(M,NN);
waktu=zeros(M,NN);

for trno=1:NN
T=data594Br(1:1501,trno);
L=max(size(T));
Tori=T;
...
options = optimoptions('ga','PopulationSize',100,'MaxGenerations',100,'Display','final');
...
plot([model Tori ]);
save(['ga_east_texas_M' num2str(M) '_N' num2str(NN) '_id' num2str(ceil(now)) ],'models','varenergy','waktu');