...
lb = [0 0.01*maxind 0.1*freqest 0.1*abs(T(maxind)) -pi];
%lb = -ones(nvars,1);
%ub = [0.09 2*maxind 16*freqest 16*abs(T(maxind))];
ub = [0.3 3*maxind 3*freqest 3*abs(T(maxind)) pi];
...
[r,fval,exitflag,out6] = simulannealbnd(fun,(lb+ub)/2,lb,ub,options);
